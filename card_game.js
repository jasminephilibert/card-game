...
while (currentIndex != 0) { rand = Math.round(Math.random() * currentIndex);
 temporaryValue = shuffledDeck[currentIndex];
 shuffledDeck[currentIndex] = shuffledDeck[rand];
 shuffledDeck[rand] = temporaryValue;
currentIndex--;
return shuffledDeck; }
...
function playGame() {
  ...
  console.log(
      deck.length != 0 ? `Good job, ${name}! You won!` : `Sorry, ${name}. You lost.`
  )
}
playGame()}
